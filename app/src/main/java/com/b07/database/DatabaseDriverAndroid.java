...
public class DatabaseDriverAndroid extends SQLiteOpenHelper {
  ...
  protected long insertAccount(int userId, boolean active) {
     ...
     * @return
     */
    protected long insertDiscount(int userId, BigDecimal amount) {
        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put("USERID", userId);
        contentValues.put("AMOUNT", amount.toString());
        long id = sqLiteDatabase.insert("DISCOUNTS", null, contentValues);
        sqLiteDatabase.close();
        return id;
    }
  ...
  protected Cursor getDiscount(int userId) {
    SQLiteDatabase sqLiteDatabase = getReadableDatabase();
    String sql = "SELECT AMOUNT FROM DISCOUNTS WHERE USERID = ?";
    return sqLiteDatabase.rawQuery(sql,new String[]{String.valueOf(userId)});
  }

  protected Cursor getUsersByRole(int roleId) {
    SQLiteDatabase sqLiteDatabase = getReadableDatabase();
      String sql = "SELECT USERID FROM USERROLE WHERE ROLEID = ?";
      return sqLiteDatabase.rawQuery(sql,
  ...
  protected Cursor getItemIdByName(String name) {
    SQLiteDatabase sqLiteDatabase = getReadableDatabase();
  ...
  protected Cursor getUserAccounts(int userId) {
    SQLiteDatabase sqLiteDatabase = getReadableDatabase();
    String sql = "SELECT ID, ACTIVE FROM ACCOUNT WHERE USERID = ?;";
    Cursor cursor = sqLiteDatabase.rawQuery(sql ,
            new String[] {String.valueOf(userId)});
    return cursor;
  }
  ...
  protected Cursor getUserActiveAccounts(int userId) {
    SQLiteDatabase sqLiteDatabase = getReadableDatabase();
    String sql = "SELECT * FROM ACCOUNT WHERE USERID = ? and ACTIVE = ?";
    Cursor cursor = sqLiteDatabase.rawQuery(sql,
  ...
  protected boolean deleteCart(int accountId) {
    SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
    System.out.println(accountId);
    boolean result = sqLiteDatabase.delete("ACCOUNTSUMMARY", "ACCTID=?",
            new String[] {String.valueOf(accountId)}) > 0;
    sqLiteDatabase.close();
    return result;
  }

    protected boolean removeDiscount(int userId) {
      SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
  boolean result = sqLiteDatabase.delete("DISCOUNTS", "USERID=?",
